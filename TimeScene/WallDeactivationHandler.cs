...
public class WallDeactivationHandler : MonoBehaviour
{
    ...
    private void OnDisable()
    {
        ...
        foreach (var placedFurniture in placedFurnitures)
        {
            ...
            if (!string.IsNullOrEmpty(placedFurniture.furnitureData.itemID))
            {
                InventoryManager.Instance?.AddFurniture(placedFurniture.furnitureData.itemID, 1);
            }

            FurnitureSaveManager.Instance?.RemoveFurniture(placedFurniture);
