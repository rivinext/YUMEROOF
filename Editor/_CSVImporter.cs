...
public class CSVImporter : EditorWindow
{
    ...
    private string outputPath = "Assets/Resources/Data/ScriptableObjects/";
    ...
    void ImportFurnitureData()
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            // ★追加（Y列＝values[1] が空ならスキップ）
            if (string.IsNullOrWhiteSpace(values[1]))
            {
                Debug.LogWarning($"Skipped row {i + 1} because NameID (column Y) is empty.");
                continue;
            }
